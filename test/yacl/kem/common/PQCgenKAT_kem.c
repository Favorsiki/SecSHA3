...
int top()
{
    ...
    int                 ret_val;
    fp = freopen("kyber512.log","w+",stdout);
    if (NULL == fp) {
    ...
    randombytes_init(entropy_input, NULL, 256);     // 1. 生成随机数，并在req文件中初始化密钥封装信息
    for (int i=0; i<20; i++) {
    ...
    do {
        ...
        printf("kem_keygen:\n");
        if ( (ret_val = crypto_kem_keypair(pk, sk)) != 0) {             // 密钥生成
        ...
        printf("\n");
        printf("kem_enc:\n");
        if ( (ret_val = crypto_kem_enc(ct, ss, pk)) != 0) {            // 加密
        ...
        printf("kem_dec:\n");
        if ( (ret_val = crypto_kem_dec(ss1, ct, sk)) != 0) {         // 解密
